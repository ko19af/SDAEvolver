...
int main(int argc, char* argv[]) {
    ...
    // initialize the variables in case none are passed on the cmd line
    ...
    for (int t = 0; t < 5; t++){
        ...
        for (int x = 0; x < hyperParameters[10]; x++){
            MyFile << "Run: " << x + 1 << endl;
        ...
        if(atoi(argv[13])){// if performing attack simulations
            AttackSim(T, hyperParameters, fName, params, atoi(argv[14]), atoi(argv[15]) / 100);
        }
